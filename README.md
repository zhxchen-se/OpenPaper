...
> 兼容旧用法：`python backend/server.py`、`python -m backend.server` 或 `uv run openpaper`。
...
注册成功后日志默认输出到 `watchdog.log`。如果你是从旧版本升级，仍然可能看到历史遗留的 `waatchdog.log`。
...
|   |-- quick_reading.py  # AI speed-read pipeline
...
|   |-- start_server.vbs
|   `-- start_waatchdog.vbs   # legacy launcher kept for backward compatibility
|-- papers/
|   |-- demo/
|   |-- metadata.json
|   |-- stats.data.json
...
| `python -m backend.server` | 兼容旧入口，效果等同于上面两种 |
...
**Q：放入 PDF 后页面没有更新？**  
A：先确认服务是通过 `python -m backend` 正在运行，再刷新浏览器；如果仍未更新，就查看 `watchdog.log`（旧版本可能仍在写 `waatchdog.log`）。
**Q：会议/年份识别不对怎么办？**  
...
如果这些工具未安装，普通论文管理和检索仍可使用，但 AI 速读会缺少页面抽取/截图能力，或者退回到纯文本模式。

常见安装方式：

- Windows：安装带 Poppler 工具链的软件发行版，例如 MiKTeX，或单独安装 Poppler 并把其 `bin` 目录加入 `PATH`
